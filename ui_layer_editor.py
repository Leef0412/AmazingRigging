...
class AMAZING_RIGGING_SplitRule(PropertyGroup):
    ...
    version: IntProperty(name="Rule Version", default=0)  # 规则版本号，用于检测是否需要迁移
...
def _check_and_migrate_rules(arm_data):
    ...
    # 空规则列表，创建默认规则
    if rule_count == 0:
        _create_default_rules_in_editor(arm_data)
        return
    
    # 获取版本号
    current_version = 0
    try:
        current_version = rules[0].version
    except AttributeError:
        current_version = 0
    
    # 版本已是最新，跳过
    if current_version >= _CURRENT_RULES_VERSION:
        return
    
    # 版本号存在但低于当前版本，迁移
    if current_version > 0 and current_version < _CURRENT_RULES_VERSION:
        print(f"[DEBUG] {arm_data.name} 规则版本 {current_version} < {_CURRENT_RULES_VERSION}，执行迁移")
        _create_default_rules_in_editor(arm_data)
        return
    ...
    old_patterns = [
        {"Ctrl Bones", "Other (Deform Bones)"},
        {"Controller", "Settings", "Support & MCH", "Deform", "Other"},
    ]
    ...
    if is_old_default:
        print(f"[DEBUG] {arm_data.name} 检测为旧版默认规则，执行迁移")
        _create_default_rules_in_editor(arm_data)
    else:
        # 用户自定义规则，仅更新版本号
        print(f"[DEBUG] {arm_data.name} 非默认规则，仅更新版本号")
        for rule in rules:
            try:
                rule.version = _CURRENT_RULES_VERSION
            except Exception:
                pass

def _create_default_rules_in_editor(arm_data):
    ...
    rule1.version = _CURRENT_RULES_VERSION
    ...
    rule2.version = _CURRENT_RULES_VERSION
    ...
    rule3.version = _CURRENT_RULES_VERSION
    ...
    rule4.version = _CURRENT_RULES_VERSION
    ...
    rule5.version = _CURRENT_RULES_VERSION
    ...
    rule_other.version = _CURRENT_RULES_VERSION
    
    print(f"[DEBUG] 已为 {arm_data.name} 创建 {len(rules)} 个默认规则 (版本 {_CURRENT_RULES_VERSION})")
...
class AMAZING_RIGGING_OT_init_data(Operator):
    ...
    def execute(self, context):
        ...
        # 检查 split rules 版本并迁移（如果需要）
        _check_and_migrate_rules(arm_data)
        ...
        if other_rule is None:
            ...
            other_rule.version = _CURRENT_RULES_VERSION
