bl_info = {
    ...
    "version": (1, 0, 0),
    "blender": (5, 0, 0),
    "location": "View3D > Sidebar > Amazing Rigging / Properties > Armature Data",
    "description": "Bone Collection visual management and rigging workflow tool",
    "doc_url": "https://github.com/lifang/AmazingRigging",
    "tracker_url": "https://github.com/lifang/AmazingRigging/issues",
    "support": "COMMUNITY",
    "category": "Rigging",
}
...
def on_load_post(dummy):
    ...
    # 延迟执行 split rules 初始化和迁移（确保 armature 数据已完全加载）
    def delayed_rules_init():
        try:
    ...
    bpy.app.timers.register(delayed_rules_init, first_interval=1.0)
...
def _create_default_rules(armature):
    ...
    rule3.version = CURRENT_RULES_VERSION
    for prefix in ("MCH-", "TGT-", "PTR-", "HLP-", "STR-"):
        p = rule3.prefixes.add()
        p.value = prefix

    # 创建默认规则 4: Settings & Val
    rule4 = rules.add()
    rule4.name = "Settings & Val"
    rule4.is_hidden = False
    rule4.version = CURRENT_RULES_VERSION
    for prefix in ("SET-", "DRV-", "VAL-"):
        p = rule4.prefixes.add()
        p.value = prefix

    # 创建默认规则 5: Visual & Props
    rule5 = rules.add()
    rule5.name = "Visual & Props"
    rule5.is_hidden = False
    rule5.version = CURRENT_RULES_VERSION
    for prefix in ("VIS-", "WGT-", "PROP-", "SKT-"):
        p = rule5.prefixes.add()
        p.value = prefix
    e = rule5.exact_matches.add()
    e.value = "Root"

    # 自动创建 "Other" 规则（用于收集未匹配的骨骼集合）
    rule_other = rules.add()
...
def initialize_default_split_rules():
    ...
    try:
        armature_count = len(bpy.data.armatures)
        print(f"[DEBUG] bpy.data.armatures 中共 {armature_count} 个 armature")
        
        for armature in bpy.data.armatures:
            arm_name = armature.name
            print(f"[DEBUG] 检查 armature: {arm_name}")
            
            # 检查是否有 amazing_split_rules 属性
            if not hasattr(armature, "amazing_split_rules"):
                print(f"[DEBUG] 跳过 {arm_name}: 缺少 amazing_split_rules 属性")
                continue
            
            rules = armature.amazing_split_rules
            rule_count = len(rules)
            print(f"[DEBUG] {arm_name} 有 {rule_count} 个规则")
            
            # 如果规则列表为空，直接创建默认规则
            if rule_count == 0:
                print(f"[DEBUG] {arm_name} 规则为空，创建默认规则")
                _create_default_rules(armature)
                continue
            
            # 检测规则版本号
            # 尝试获取第一个规则的 version 属性
            first_rule = rules[0]
            current_version = 0
            try:
                current_version = first_rule.version
            except AttributeError:
                current_version = 0
            
            print(f"[DEBUG] {arm_name} 规则版本: {current_version}, 当前最新版本: {CURRENT_RULES_VERSION}")
            
            # 版本已是最新，跳过
            if current_version >= CURRENT_RULES_VERSION:
                print(f"[DEBUG] 跳过 {arm_name}: 规则版本 {current_version} 已是最新")
                continue
            
            # 版本低于当前版本，需要迁移
            if current_version > 0 and current_version < CURRENT_RULES_VERSION:
                print(f"[DEBUG] {arm_name} 规则版本 {current_version} < {CURRENT_RULES_VERSION}，执行迁移")
                _create_default_rules(armature)
                continue
            
            # version == 0，旧数据没有版本号，通过规则名称判断
            rule_names = {rule.name for rule in rules}
            print(f"[DEBUG] {arm_name} 旧规则名称集合: {rule_names}")
            
            # 已知旧版本规则名称模式
            old_patterns = [
                {"Ctrl Bones", "Other (Deform Bones)"},  # 版本 1
                {"Controller", "Settings", "Support & MCH", "Deform", "Other"},  # 版本 2
            ]
            
            is_old_default = False
            for old_pattern in old_patterns:
                # 如果规则名称完全匹配旧模式，或者是旧模式的子集
                if rule_names == old_pattern or rule_names.issubset(old_pattern):
                    is_old_default = True
                    break
                # 如果有交集（包含旧规则名称），也可能是旧版本
                if rule_names & old_pattern and len(rule_names) <= len(old_pattern):
                    is_old_default = True
                    break
            
            if is_old_default:
                print(f"[DEBUG] {arm_name} 检测为旧版默认规则，执行迁移")
                _create_default_rules(armature)
            else:
                # 用户自定义的规则，不覆盖，但标记版本号
                print(f"[DEBUG] 跳过 {arm_name}: 非默认规则（可能是用户自定义），仅更新版本号")
                for rule in rules:
                    try:
                        rule.version = CURRENT_RULES_VERSION
                    except Exception:
                        pass
